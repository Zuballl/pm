...
def get_slack_channels(
    ...
    """
    Get a list of channel names in the project's Slack workspace.
    """
    return services_slack.get_channels(db, project_id)


@router.post("/api/slack/send_message")
def send_slack_message(
    ...
    """
    return services_slack.send_message(db, project_id, channel_id, message)
...
def get_slack_messages(
    ...
    """
    Retrieve past messages from a Slack channel.
    """
    return services_slack.get_messages(db, project_id, channel_id)
...
@router.get("/api/debug/access_token")
def debug_access_token(project_id: int, db: orm.Session = fastapi.Depends(database.get_db)):
    try:
        token = services_slack.get_slack_token(db, project_id=project_id)
